...
int main () {   
    ...
    cout<< output(state)<<endl; //print the output of the starting state
    ...
    for(int k=0;k<n;k++){ 
        ...
        if (nextstate < 0){ //check the combination of the current state and the input is correct
            ...
            if(state != nextstate){ //print the output if the state is changed
                cout<< output(nextstate)<<endl;
            } 
            state=nextstate;
            /*macchina mealy
             if(state == nextstate){
                cout<<"stato rimasto a s"<<state<<enl;
            }else{
                cout<<"stato cambiato a s"<<state<<endl; 
            } */            
        }        
    }        
